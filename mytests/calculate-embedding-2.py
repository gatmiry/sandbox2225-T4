...
def calculate_embedding(input_text):
    with torch.no_grad():
        ...
        hidden_embedding = model.encoder(torch.tensor(outputs['input_ids'])).last_hidden_state.detach()[0, :outputs['input_ids'].shape[1]-2, :]
    #return hidden_embedding
    return torch.mean(hidden_embedding, dim=0)




# a lot more similar a & b!
...
# a little more similar a & b! (about urban life)
...
# a little more similar a & b! (about art), 
...
## a & b a little more similar (stories about egyptian temples) l-infinity works better than l-2
