...
ds = DatasetDict({
    'validation': similar_test_dataset,
})
...
def tokenize(element):
    bool_array = istring(np.array(element['similar'])) & istring(np.array(element['random']))
    base_questions = [mylist[qid_dict[qid]] for qid in np.array(element['qid'])[bool_array]]
    similar_questions = [[mylist[qid_dict[int(num)]] for num in x.split()] 
                             for x in np.array(element['similar'])[bool_array]]
    random_questions = [[mylist[qid_dict[int(num)]] for num in x.split()] 
                             for x in np.array(element['random'])[bool_array]]
...
tokenized_datasets = ds.map(
    tokenize, batched=True, remove_columns=ds["validation"].column_names, batch_size=10
)
#similar_questions =  
...
tokenized_datasets.save_to_disk('./model3-outputs-gpt2tokenizer-askubuntubody-margintesting')